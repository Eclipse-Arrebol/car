...
def test_bpr_responds_to_active_flow(bpr_env):
    ...
    # 本测只验证「边上 EV active count → BPR」；背景聚合流由 test_background_traffic 覆盖。
    saved_base = env.background_edge_base_flows
    saved_flows = env.background_edge_flows
    env.background_edge_base_flows = {}
    env.background_edge_flows = {}
    try:
        t_low, t0_h = _t_h_from_dynamic(env, u, v, 0)
        assert t0_h > 0
        assert abs(t_low - t0_h) / t0_h <= 0.001, "零 EV 流且无背景流时 t_h 应与 t0_h 一致（≤0.1% 误差）"
        ...
        env.background_edge_flows = saved_flows
